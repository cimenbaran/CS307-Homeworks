...
public class Philosopher extends Thread {
	...
	public static void main(String args[]) {
		...
		for (int i=0; i < N; i++)
		{
			...
			oldies[i].start();
			try {
				sleep(800); // since 5000 was too long I reduced it to 800
			} catch (InterruptedException e) {
		...
		for (int i=0; i<5; i++) {
			while (!bararray[i].hasQueuedThreads()) {}
		}
		for (int i=0; i<5; i++) {
			bararray[i].release();
		}
	...
	public void try_taking_forks_goes_brrrrr(int id) {
		if( state[id] == HUNGRY && state[(N+id-1) % 5] != EATING && state[(id+1) % 5] != EATING) {
			state[id] = EATING;
	...
	public void think_goes_brrrrr() {
		Random random_think  = new Random();
		int upperbound = 10;
		int lowerbound = 0;
		int thinkTime = (random_think.nextInt(upperbound-lowerbound) + lowerbound) * 1000;
		try {
			sleep(thinkTime);
		} catch(InterruptedException e) {
			e.printStackTrace();
	...
	public void take_forks_goes_brrrrr(int id) {
		try {
		...
		state[id] = HUNGRY;
		table.Hungry_GUI(id);
		try_taking_forks_goes_brrrrr(id);
		mutex.release();
	...
	public void put_forks_goes_brrrrr(int id) {
		try {
			mutex.acquire(); // enter critical region
		} catch(InterruptedException e) {
			e.printStackTrace();
		...
		try_taking_forks_goes_brrrrr((N+id-1) % 5 );
		try_taking_forks_goes_brrrrr((id+1) % 5 );
	...
	public void run() {
		...
		//int arrivalTime = (random.nextInt((N-1-id)*5 +upperbound-lowerbound) + lowerbound) * 1000; // didn't used it, instead reduced the sleep time to 800 from 5000

		int arrivalTime = (random.nextInt(upperbound-lowerbound) + lowerbound) * 1000;
		try {
			sleep(arrivalTime);
		} catch(InterruptedException e) {
			e.printStackTrace();
		...
		//System.out.println(id + " " + arrivalTime); // Debug

		synchronized (barriers[id]) {
			try {
				barriers[id].acquire();
			} catch (InterruptedException e) {
				e.printStackTrace();
		...
		while(true) {
			think_goes_brrrrr();
			take_forks_goes_brrrrr(id);
			table.Eating_GUI(id);
			put_forks_goes_brrrrr(id);
